...
4. `abacus-librpa-2026-09-03-strict2d-sos-rpa-v2` is `ENABLED` only for benchmark-bound `strict_2d_sos_rpa`. Call `inspect_route_benchmark`, `evaluate_route_benchmark`, and `evaluate_route_benchmark_suite` for `strict2d-sos-rpa-mos2-qavg-v1`. This reference-bounded result makes no asymptotic exponent claim and is not strict-2D GW acceptance.
