...
3. Default `abacus-librpa-2026-09-06-v5` requires reader v1 and `stru_out`; periodic 3D GW is L3 `EXPERIMENTAL`: fixed `nfreq=24 -> 32` and BN screening `12x12x12 -> 14x14x14` pass; symmetry/full-q, basis, transfer, and reference gates remain; molecular Delta-Sternheimer RPA and solid Delta-Sternheimer RPA routes await the metric. Keep `abacus-librpa-2026-09-03-v4` and `abacus-librpa-2026-08-30-v2` historical; use `abacus-librpa-2026-08-30-v3` only for that metric.
4. `abacus-librpa-2026-09-03-strict2d-sos-rpa-v2` is `ENABLED` only for reference-bounded `strict2d-sos-rpa-mos2-qavg-v1`. Use `inspect_route_benchmark`, `evaluate_route_benchmark`, and `evaluate_route_benchmark_suite`; it proves no asymptotic exponent and is not strict-2D GW acceptance.
