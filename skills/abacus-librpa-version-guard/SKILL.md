...
| `abacus-librpa-2026-09-06-v5` | `1648a8a344427ae1b6394912bf677c4a20e053f2` | `7e40c5bbf735a78aa15fa589ca2468fec2e2427b` | immutable pre-symmetry-control profile |
...
`solid_delta_st_rpa` as admission routes. In v6, only periodic 3D GW is
`EXPERIMENTAL`; none is `ENABLED`. Use v6 for new work. Keep v5, v4, and
...
- For v4/v5/v6 periodic 3D GW, explicitly set `tfgrids_type = minimax`,
...
- V6 additionally records the BN `4x4x4`, no-head/wing
  `8-q symmetry -> 64-q full-q` comparison as `PASS_REFERENCE_BOUNDED` at
  `0.0001 eV`. Its complete 78-state maximum difference is `1e-5 eV`.
  This does not close empty-state, NAO, ABFS, transfer, or physical-reference
  gates, so aggregate scientific acceptance remains `NOT_EVALUATED`.
