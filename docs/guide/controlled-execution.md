...
OML 0.4.10 provides bounded MCP execution and scientific finalization for one route: **non-SOC periodic GW** with ABACUS reader-v1, PyATB head/wing, and LibRPA 0.7.0. It does not expose arbitrary shell, SSH, Slurm options, cleanup, repair, or retry commands.
...
OML 0.4.10 never reruns a terminal stage in the same run directory. Failed logs and artifacts remain available for diagnosis; no controlled retry cleans or silently reuses them. If initial synchronization fails, the registered run becomes `PREPARE_FAILED`; it is retained for diagnosis and cannot be represented as successfully prepared.
...
`finalize_case` evaluates the full low-energy interval from `VBM-3` through `CBM+3` at every band-path k point. Definition-matched regression requires KS, EXX, and GW state errors no larger than `0.001 eV`. Convergence requires isolated symmetry, `nfreq`, empty-state, and screening-k-grid axes. Symmetry equivalence gates KS, EXX, GW, and the fundamental gap at its axis-specific `0.0001 eV` tolerance; the other current axes use `0.05 eV`. Definition schema 2 also fixes the frequency-grid, analytic-continuation, QP-solver, and LibRPA symmetry controls, so a comparison cannot silently change another numerical definition.
...
group mean is preserved. This pattern is reported as
`BLOCKED_DEGENERATE_GAUGE_MISMATCH`, remains `FAIL`, and requires explicit
wavefunction-subspace verification. The diagnostic never converts a historical
statewise mismatch into scientific acceptance.

The empty-state axis has one finite-basis endpoint rule. When both runs use the
same positive `band_out` basis dimension, the coarse run uses fewer states, and
the fine run has `nbands` exactly equal to that basis dimension, the axis passes
as `COMPLETE_BASIS_STATE_SPACE`. No additional empty state exists in that fixed
NAO Hilbert space, so asking for a larger `nbands` would be invalid. The measured
state and gap changes remain in the report. This closes only empty-state
truncation for the fixed basis; NAO basis completeness is a separate, still
unevaluated basis-ladder gate.

Scientific reports inherit convergence axes only from reports produced by the
same evaluator version. This prevents a changed scientific rule from silently
mixing old and new verdicts in one report.
